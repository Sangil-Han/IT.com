	...
	<div id="wrap">
		...
		<form action="/finish/search.do" method="get">
			<select name="searchOption">
				...
				<option value="all"
					<c:if test="${searchOption eq 'all'}">selected</c:if>>전체
				</option>
				<option value="center"
			...
			<tr align="center" height="20">
				<td colspan="6">
					<c:if test="${startNavi ne 1}">
						<a href="/finish/${urlVal }.do?page=${startNavi-1}&searchOption=${searchOption}&serachValue=${searchValue}">[이전]</a>
					</c:if>
					...
					<c:if test="${endNavi ne maxPage }">
						<a href="/finish/${urlVal }.do?page=${endNavi+1 }&searchOption=${searchOption}&serachValue=${searchValue}">[다음]</a>
					</c:if>
				</td>
			</tr>
		</table>
